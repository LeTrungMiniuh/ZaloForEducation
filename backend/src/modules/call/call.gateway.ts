...
export class CallGateway {
  ...
  handleCallInvite(
    ...
    if (!data?.convId || !data?.callId) return;

    const engine = "chime";

    const recipients = Array.from(
      new Set(
        (data.toEmails && data.toEmails.length > 0
          ? data.toEmails
          : data.toEmail
            ? [data.toEmail]
            : []
    ...
    this.logger.log(
      `[Invite] ${data.fromEmail} → ${recipients.join(", ")} | CallId: ${data.callId} | Engine: ${engine}`,
    );

    recipients.forEach((email) => {
      const targetRoom = `user#${email}`;
  ...
  async handleCallAccept(
    ...
    this.logger.log(
      `[Accept] notifying caller ${data.toEmail} | CallId: ${data.callId}`,
    );
    ...
    this.callService
      .markCallStarted(data.callId)
      .catch((e) => this.logger.error("Failed to mark call start", e));
    ...
    if (userEmail) {
      ...
      this.logger.log(
        `[Dismiss] notifying other devices of ${userEmail} via room: ${myRoom}`,
      );
      ...
      client.broadcast.to(myRoom).emit("call:dismiss", {
        convId: data.convId,
        callId: data.callId,
        reason: "accepted",
      });
    ...
    await this.callService["redis"].set(
      `call:state:${data.callId}`,
      "ACCEPTED",
      1800,
    );
  ...
  async handleCallReject(
    @MessageBody()
    data: { convId: string; toEmail: string; callId: string; reason?: string },
    @ConnectedSocket() client: Socket,
  ) {
    if (!data?.convId || !data?.toEmail) return;
    const callerRoom = `user#${data.toEmail.toLowerCase()}`;
    ...
    (async () => {
      try {
        ...
        const receiver = session
          ? initiator === session.initiatorEmail
            ? fromEmail
            : session.initiatorEmail
          : fromEmail;

        await this.callService.hangupMeeting(
          data.convId,
          data.callId,
          fromEmail,
        );
        ...
        if (callMsg) {
          ...
          this.server
            .to(`user#${initiator.toLowerCase()}`)
            .emit("receiveMessage", callMsg);
          this.server
            .to(`user#${receiver.toLowerCase()}`)
            .emit("receiveMessage", callMsg);
        }
      } catch (e) {
  ...
  async handleCallTimeout(
    ...
    recipients.forEach((email) => {
      ...
      this.server
        .to(targetRoom)
        .emit("call:timeout", { convId: data.convId, callId: data.callId });
      this.server.to(targetRoom).emit("call:dismiss", {
        convId: data.convId,
        callId: data.callId,
        reason: "timeout",
      });
      this.server.to(targetRoom).emit("call:handled_elsewhere", {
        convId: data.convId,
        callId: data.callId,
        reason: "timeout",
      });
  ...
  handlePeerJoined(
    ...
    if (timeout) {
      ...
      this.waitForJoinTimeouts.delete(data.callId);
      this.logger.log(
        `[PeerJoined] Cleared Ghost Hangup timeout for CallId: ${data.callId}`,
      );
    }
    ...
    recipients.forEach((email) => {
      ...
      this.server
        .to(targetRoom)
        .emit("call:peer_joined", { convId: data.convId, callId: data.callId });
    });
  }
  ...
  async handleHangup(
    ...
    this.logger.log(
      `[Hangup] ${data.convId} | CallId: ${data.callId} | Dur: ${data.duration} | Type: ${data.callType}`,
    );
    const fromEmail = client["user"]?.email || "system";
    ...
    (async () => {
      try {
        ...
        const { session } = await this.callService.hangupMeeting(
          data.convId,
          data.callId,
          fromEmail,
        );
        ...
        const callType =
          data.callType || (session?.callType as "audio" | "video") || "audio";

        let receiver = recipients[0] || data.toEmail || "unknown";
        if (session && session.initiatorEmail === fromEmail) {
          receiver = recipients[0] || data.toEmail || "unknown";
        } else if (session) {
          receiver = fromEmail;
        }
  ...
  handleUpgradeRequest(
    ...
    this.logger.log(
      `[UpgradeRequest] → ${data.toEmail} | CallId: ${data.callId}`,
    );
  ...
  handleUpgradeAccepted(
    ...
    const targetRoom = `user#${data.toEmail.toLowerCase()}`;
    this.logger.log(
      `[UpgradeAccepted] → ${data.toEmail} | CallId: ${data.callId}`,
    );
  ...
  handleUpgradeDeclined(
    ...
    const targetRoom = `user#${data.toEmail.toLowerCase()}`;
    this.logger.log(
      `[UpgradeDeclined] → ${data.toEmail} | CallId: ${data.callId}`,
    );
