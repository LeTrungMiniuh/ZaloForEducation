...
const ChatInput: React.FC<ChatInputProps> = ({
  ...
  const liveLocationTimerRef = useRef<ReturnType<typeof setInterval> | null>(
    null,
  );
  const liveLocationStopRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const contactCandidates = acceptedFriends
    .map((friendship) =>
      friendship.sender_id === user?.email
        ? friendship.receiver_id
        : friendship.sender_id,
    )
    .filter(
      (email, index, arr) =>
        !!email && email !== user?.email && arr.indexOf(email) === index,
    );
        ...
        currentAttachments.map(async (item) => {
          ...
          if (isImage && !sendImageAsHD && item.mimeType !== "image/gif") {
            try {
              ...
              console.warn(
                "Image compression failed, fallback to original file:",
                compressError,
              );
            }
          }
  ...
  const sendVoiceBlob = async (blob: Blob, durationSec: number) => {
    try {
      ...
      Swal.fire(
        "Lỗi",
        "Không thể gửi tin nhắn thoại. Vui lòng thử lại.",
        "error",
      );
      return false;
    } finally {
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      Swal.fire(
        "Không thể thu âm",
        "Bạn cần cho phép quyền microphone để gửi tin nhắn thoại.",
        "warning",
      );
      cleanupRecordingResources();
    }
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () =>
        Swal.fire(
          ...
          "Vui lòng bật quyền truy cập vị trí và thử lại.",
          "warning",
        ),
  ...
  const openContactPicker = () => {
    if (contactCandidates.length === 0) {
      Swal.fire(
        "Chưa có liên hệ",
        "Bạn cần kết bạn để gửi danh thiếp liên hệ.",
        "info",
      );
      return;
    }
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (attachments.length + validFiles.length > 50) {
      Swal.fire(
        "Quá giới hạn",
        "Chỉ được tải lên tối đa 50 tệp/ảnh cùng một lúc.",
        "warning",
      );
  ...
  return (
    <div className="p-2 bg-white/80 dark:bg-surface-container/90 backdrop-blur-xl border-t border-outline-variant/10 dark:border-outline-variant/30 space-y-1.5 shadow-[0_-4px_24px_rgba(0,0,0,0.03)] dark:shadow-[0_-4px_24px_rgba(0,0,0,0.25)] z-10 transition-all duration-300">
      ...
      {attachments.length > 0 && (
        <div className="flex flex-wrap gap-2 py-1 animate-in fade-in slide-in-from-left-2 duration-300 max-h-[170px] overflow-y-auto custom-scrollbar">
          {attachments.map((a, i) => (
            <div
              key={i}
              className="relative group/att shadow-sm hover:shadow-md transition-all duration-300"
            >
              <div className="w-16 h-16 rounded-xl bg-surface-container-low border border-outline-variant/10 flex items-center justify-center overflow-hidden relative">
                {a.mimeType.startsWith("image/") ? (
                  <img
                    src={a.dataUrl}
                    className="w-full h-full object-cover group-hover/att:scale-110 transition-transform duration-500"
                    alt=""
                  />
                ) : (
                  <div className="flex flex-col items-center gap-1">
                    <FileText size={22} className="text-primary" />
                    <span className="text-[9px] font-bold text-on-surface-variant uppercase tracking-tighter truncate w-14 px-1">
                      {a.name.split(".").pop()}
                    </span>
                  </div>
                )}
                {(a.isSticker ||
                  (a.mimeType.startsWith("image/") && sendImageAsHD)) && (
      ...
      {pendingVoice && !isRecording && (
        <div className="flex items-center gap-2 p-2 rounded-xl border border-outline-variant/20 bg-surface-container-low/60">
          ...
          <div className="text-[11px] font-bold text-on-surface-variant shrink-0">
            {Math.floor(pendingVoice.durationSec / 60)
              .toString()
              .padStart(2, "0")}
            :{(pendingVoice.durationSec % 60).toString().padStart(2, "0")}
          </div>
          <button
            type="button"
      ...
      {showContactPicker &&
        ...
        createPortal(
          <div
            className="fixed inset-0 z-[1000] bg-black/40 backdrop-blur-[2px] flex items-start justify-center pt-3 p-4"
            onClick={() => setShowContactPicker(false)}
          >
            <div
              className="w-full max-w-2xl bg-white dark:bg-surface-container rounded-2xl shadow-2xl border border-outline-variant/20 overflow-hidden"
            ...
            >
              <div className="px-4 py-3 border-b border-outline-variant/10 flex items-center justify-between">
                ...
                <button
                  onClick={() => setShowContactPicker(false)}
                  className="w-8 h-8 rounded-full hover:bg-surface-container-high text-on-surface-variant"
                >
                  ×
                </button>
              </div>
              <div className="p-4 space-y-3">
                ...
                <div className="flex items-center gap-2">
                  <span className="px-3 py-1.5 rounded-full bg-primary text-white text-[13px] font-bold">
                    Tất cả
                  </span>
                ...
                <div className="max-h-[420px] overflow-y-auto pr-1 space-y-1">
                  {filteredContacts.length === 0 && (
                    <div className="py-8 text-center text-[14px] text-on-surface-variant">
                  ...
                  {filteredContacts.map((item) => (
                    ...
                    >
                      <span className="shrink-0 w-5 h-5 rounded-full border-2 border-outline-variant/60 flex items-center justify-center">
                        {selectedContactEmail === item.email ? (
